  ...
  <div class="app-container">
    ...
    <el-form :inline="true" class="demo-form-inline">
      <el-form-item label="手机号">
        <el-input v-model="userInfoQuery.mobile" placeholder="手机号" />
      </el-form-item>

      <el-form-item label="用户类型">
        <el-select v-model="userInfoQuery.userType" placeholder="请选择" clearable>
      ...
      <el-form-item label="用户状态">
        <el-select v-model="userInfoQuery.status" placeholder="请选择" clearable>
          <el-option label="正常" value="1" />
          <el-option label="锁定" value="0" />
        </el-select>
      </el-form-item>

      <el-button type="primary" icon="el-icon-search" @click="pages()">
        查询
      </el-button>
    ...
    />
    <!-- 用户登录日志 -->
    <el-dialog title="User login logs" :visible.sync="dialogTableVisible">
...
export default {
  ...
  methods: {
    ...
    showLoginRecord(id) {
      ...
      //加载数据列表
      userInfo.getUserLoginRecordTop50(id).then(response => {
    ...
    // 锁定和解锁用户
    lock(id, status) {
      userInfo.lock(id, status).then(response => {
        if (status === 0) {
          this.$message.warning(response.message)
        } else {
          this.$message.success(response.message)
        }
        // 解锁和锁定之后都调用分页方法及时刷新页面
        this.pages()
      })
    },
    pages() {
      userInfo.getPageList(this.page,this.limit,this.userInfoQuery).then(response => {
    ...
    // 重置表单
    resetData() {
      this.userInfoQuery = {} // 清空查询条件框中的值
      this.pages() // 清空完毕后调用分页方法并刷新列表
    }
  }
}
</script>
