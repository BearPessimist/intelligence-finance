...
export default {
  ...
  created() {
    if (this.$route.params.id) { // 如果路径中有id值
      this.fetchDataById();  // 调用分页方法
    }
  },
  methods: {
    fetchDataById() {     // 参数id填写路由路径中的id值
      borrowerApi.show(this.$route.params.id).then(response => {
        this.borrower = response.data.borrowerDetailVO;
      })
    },
    back() { // 调用back方法，返回到列表页面
      this.$router.push({ path: '/core/borrower/list' })
    },
    // 借款额度审核方法
    approvalSubmit() {
      this.saveBtnDisabled = true;
      this.approvalForm.borrowerId = this.$route.params.id; // 赋值借款人的id
