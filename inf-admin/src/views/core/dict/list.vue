<template>
  <div class="app-container">
    ...
    <el-table :data="dictList" border row-key="id" lazy :load="getChildren">
...
export default {
  ...
  data() {
    return {
      ...
      BASE_API: process.env.VUE_APP_BASE_API, //获取后端接口地址，等于http://localhost
      dictList: [],

    }
  },
  created() {
    this.listTreeDictData();
  },
  methods: {
    ...
    fileUploadSuccess(response) {
      if (response.code === 200) { // 如果响应码为200，上传成功
        this.$message.success('数据导入成功') // 调用此消息
        this.listTreeDictData();
        this.dialogVisible = false // 关闭dialog对话框
      } else {
        this.$message.error(response.message) // 否则抛出错误
      }
    },
    //上传失败回调
