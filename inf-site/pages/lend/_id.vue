<template>
  ...
  <div class="item-detail wrap">
    ...
    <div class="item-detail-head clearfix" data-target="sideMenu">
      ...
      <!-- 投资表单开始，用户类型等于1，也就是说如果是投资人则再展示以下信息。 -->
...
export default {
  ...
  async asyncData({ $axios, params }) {
    ...
    return { // 返回后端接口赋的值
      lend: response.data.lendDetail.lend, // 赋值标的详情
      borrower: response.data.lendDetail.borrower, // 赋值借款人信息
  ...
  data() {
    return {
      ...
      lendItemReturnList: [], // 回款计划 全局变量存放
    }
  },
  ...
  methods: {
    ...
    fetchLendItemReturnList() {
      this.$axios.$get('/api/core/lendItemReturn/auth/list/' + this.$route.params.id).then((response) => {
    ...
    fetchAccount() {
     ...
     if (userInfo) { // 如果用户信息存在
       this.$axios.$get(`/api/core/userAccount/auth/getAccount`).then((response) => {
         this.account = response.data.account; // 账户余额的赋值。
       })
     }
    },
    // 获取登录人的用户类型，是投资人还是借款人
    fetchUserType() {
      let userInfo = cookie.get('userInfo');
      if (userInfo) { // 有cookie再进行下面的解析和赋值的操作。
        userInfo = JSON.parse(userInfo); // 将json字符串解析为对象
        this.userType = userInfo.userType; // 将cookie中的用户类型赋值到当前页面下全局变量的用户类型变量中做渲染。
        this.user = userInfo;
      }
    },
    ...
    getInterestCount() {
      this.$axios.$get(`/api/core/lend/getInterestCount/${this.invest.investAmount}/${this.lend.lendYearRate}/${this.lend.period}/${this.lend.returnMethod}`).then((response) => {
        this.interestCount = response.data.interestCount; // 进行赋值的操作。
      })
    },
    //投资
    commitInvest() {
      ...
      //校验当前用户是否是投资人
      let userInfoObj = JSON.parse(userInfo) // 解析cookie信息即可。
          ...
          '前往汇付宝资金托管平台',
          {
            ...
            callback: (action) => {
              ...
              if (action === 'confirm') {
                ...
                this.$axios.$post('/api/core/lendItem/auth/commitInvest', this.invest).then((response) => {
