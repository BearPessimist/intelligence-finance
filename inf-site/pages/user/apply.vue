...
export default {
  ...
  watch: { // 监听借款金额
    'borrowInfo.amount'(value) { // value为页面输入的金额数
      if (value > this.borrowAmount) {
        let _this = this; // 进行this的赋值，因为在callback使用this关键字的作用域会出问题
        this.$alert('您的借款额度不足！', {
          type: 'warning',
          // 回调参数。
          callback() { // 如果输入的额度超过最大额度，则将输入的额度改回可以借款的最大额度。
  ...
  mounted() { // 在mounted 中执行，避免拿不到cookie值导致后台报错未登录现象。mounted 函数在浏览器端中执行
  },
  created() { // created 在前端服务器端执行，渲染之前，
  ...
  methods: {
    ...
    getBorrowInfoStatus() {
      this.$axios.$get(`/api/core/borrowInfo/auth/getBorrowInfoStatus`).then(response => {
        this.borrowInfoStatus = response.data.borrowInfoStatus;// 赋值操作
        ...
        if (this.borrowInfoStatus === 0) {
          ...
          // 初始化下拉列表内容
          this.initSelected();
          this.getBorrowAmount(); // 获取额度值
        } else if(this.borrowInfoStatus === 1) {
          this.active = 1; // 展示1 认证中的状态
        } else {
          this.active = 2; // 否则就是审核通过的步骤页面。
        }
      })
    },
    ...
    initSelected() {
      // 获取还款方式列表
      this.$axios.$get('/api/core/dict/dictCode/returnMethod').then((response) => {
            this.returnMethodList = response.data.dictList
          })

      // 获取资金用途列表
      this.$axios.$get('/api/core/dict/dictCode/moneyUse').then(response => {
    ...
    getBorrowAmount() {
      this.$axios.$get('/api/core/borrowInfo/auth/getBorrowAmount').then((response) => {
        this.borrowAmount = response.data.borrowAmount; // 赋值操作
      })
    },
