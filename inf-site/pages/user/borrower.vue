...
<!--              通过:data{ module: ...} 来区分上传的是哪个证件的信息，module属性对应后端接口的@RequestParam("module") 的参数   -->
            <el-upload
...
export default {
  ...
  methods: {
    getUserInfo() {
      this.$axios.$get('/api/core/borrower/auth/getBorrowerStatus')
          .then((response) => {
            ...
            if (this.borrowerStatus === 0) {
              ...
              this.active = 1; // 切换到认证中的状态
            } else {
              this.active = 2; // 否则就是认证成功的页面
            }
          });
    },

    initSelected() {
      //学历列表，根据字典编码进行查询，因为字典编码字段是String类型的，所以URL地址可以直接写所需字段的类型
      this.$axios.$get('/api/core/dict/dictCode/education').then((response) => {
            this.educationList = response.data.dictList
          })

      //行业列表
      this.$axios.$get('/api/core/dict/dictCode/industry').then((response) => {
            this.industryList = response.data.dictList
          })

      //收入列表
      this.$axios.$get('/api/core/dict/dictCode/income').then((response) => {
            this.incomeList = response.data.dictList
          })

      //还款来源列表
      this.$axios.$get('/api/core/dict/dictCode/returnSource').then((response) => {
            this.returnSourceList = response.data.dictList; // 进行后端接口的赋值操作
          })

      //联系人关系列表
      this.$axios
          .$get('/api/core/dict/dictCode/relation')
          .then((response) => {
    ...
    onUploadRemove(file, fileList) {
      ...
      this.$axios.$delete('/api/oss/file/remove?url=' + file.response.data.url).then((response) => {
            console.log('远程删除'); // 打印日志
        // 调用filter过滤方法剔除文件，然后将过滤之后的列表再赋值到源数据当中；
        this.borrower.borrowerAttachList = this.borrower.borrowerAttachList.filter(response => {
          // 如果不相等的话就返回true，并删除文件
          return response.imageUrl !== file.response.data.url;
        })
      })
    },
  }
