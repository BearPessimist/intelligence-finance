...
export default {
  ...
  methods: {
    commitBind() {
      this.$alert(
        '<div style="size: 18px;color: #c76611;">您即将前往汇付宝绑定账号</div>',
        '前往汇付宝资金托管平台',
          {
          dangerouslyUseHTMLString: true, // true表示将上面嵌入的div解析为html 的样式，false则为原样输出
          confirmButtonText: '立即前往',
          callback: (action) => {
            //TODO 提交用户信息
            this.$axios.$post(`/api/core/userBind/auth/bind`,this.userBind).then(response => {
              // 将数据直接渲染到浏览器端
